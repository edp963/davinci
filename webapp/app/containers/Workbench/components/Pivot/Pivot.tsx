...
export interface IMetricAxisData {
  min: number
  max: number
  interval: number
}
...
export class Pivot extends React.PureComponent<IPivotProps, IPivotStates> {
  ...
  public componentWillUpdate (nextProps: IPivotProps, nextState: IPivotStates) {
    const { metrics, data, chart } = nextProps
    const { cols, rows } = nextState
    ...
    this.colTree = {}
    this.tree = {}
    this.rowHeaderWidths = rows.map((r) => getTextWidth(r, 'bold'))
    this.drawingData = {
      extraMetricCount: 0,
      elementSize: 0,
      unitMetricWidth: 0,
    ...
    if (!cols.length && !rows.length) {
      ...
      data.forEach((record) => {
        this.getPivotData(nextProps, nextState, record, !!dimetionAxis)
      })
      this.rowKeys.sort(this.sortingKeys(nextProps.rows))
      this.colKeys.sort(this.sortingKeys(nextProps.cols))
    ...
    if (dimetionAxis) {
      ...
      this.metricAxisData = metrics.reduce((obj, m, i) => {
  ...
  private getPivotData = (props: IPivotProps, state: IPivotStates, record: object, hasDimetionAxis: boolean) => {
    const { metrics } = props
    ...
    rows.forEach((r, i) => {
      ...
      const textWidth = getTextWidth(value, 'bold')
    ...
    if (colKey.length) {
      if (!this.colTree[flatColKey]) {
        const width = !hasDimetionAxis && { width: Math.max(...colKey.map((c) => getTextWidth(c, 'bold'))) }
        const height = !hasDimetionAxis && { height: PIVOT_LINE_HEIGHT }
    ...
    if (metrics.length) {
      ...
      if (!hasDimetionAxis) {
        const maxTextWidth = Math.max(...metrics.map((m) => getTextWidth(record[`${m.agg}(${decodeMetricName(m.name)})`])))
        ...
        <div className={styles.leftSide}>
          ...
          <div className={styles.rowHeader}>
            ...
            <RowHeader
              ...
              metrics={metrics}
              drawingData={this.drawingData}
              metricAxisData={this.metricAxisData}
