  ...
  produce(state, (draft) => {
    switch (action.type) {
      ...
      case ActionTypes.EDIT_SOURCE_SUCCESS:
        draft.sources.splice(draft.sources.findIndex((g) => g.id === action.payload.result.id), 1, action.payload.result)
        draft.formLoading = false
        break
