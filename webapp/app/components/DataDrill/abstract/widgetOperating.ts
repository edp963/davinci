...
import { IWidgetDimension, WidgetMode, IWidgetFilter } from 'app/containers/Widget/components/Widget'
import { OperateObjectAbstract } from 'utils/abstract/OperateObjectAbstract'
import { getTypesOfModelByKeyName, getListsByViewModelTypes} from 'containers/View/util'
...
export default class OperatingWidget extends OperateObjectAbstract {
  ...
  public widgetIntoPool (widgets) {
    const widgetPool = widgets.reduce((iteratee, widget) => {
      ...
      iteratee[Number(id)] = this.jsonParse(config) // possible performance defects
      return iteratee
    }, {})
  ...
  public initGroups () {
    ...
    if (!widget.initGroups) {
      ...
      const groups =  [
        ...
        ...compose(mappingName, setDefaultEmptyArray, getItem, setDefaultEmptyArray)(color),
        ...compose(mappingName, setDefaultEmptyArray, getItem, setDefaultEmptyArray)(label)
      ]
  ...
  public initAggregators () {
    ...
    if (!widget.initAggregators) {
      ...
      const aggregators = [
        ...
        ...compose(mappingAggregators, filterByValue, setDefaultEmptyArray, getItem, setDefaultEmptyArray)(label),
        ...compose(mappingAggregators, setDefaultEmptyArray, getItem, setDefaultEmptyArray)(size),
        ...compose(mappingAggregators, setDefaultEmptyArray, getItem, setDefaultEmptyArray)(xAxis),
        ...compose(mappingAggregators, setDefaultEmptyArray, getItem, setDefaultEmptyArray)(tip)
      ]
  ...
  public getMode () {
    return this.getTargetPropsByProperty<WidgetAbstract, 'mode'>(this.widgetProps, 'mode')
  }
  ...
  public getModel () {
    return this.getTargetPropsByProperty<WidgetAbstract, 'model'>(this.widgetProps, 'model')
  }
  ...
  public getDimetionAxis () {
    return this.getTargetPropsByProperty<WidgetAbstract, 'dimetionAxis'>(this.widgetProps, 'dimetionAxis')
  }

  public getSelectedChart () {
    return this.getTargetPropsByProperty<WidgetAbstract, 'selectedChart'>(this.widgetProps, 'selectedChart')
  }

  public getRowsOrCols (dimension: WidgetDimensions) {
    return this.getTargetPropsByProperty<WidgetAbstract, WidgetDimensions>(this.widgetProps, dimension) || []
  }
  ...
  public deleteWithSthRowsOrCols (
    ...
    const target = this.getTargetPropsByProperty<WidgetAbstract, WidgetDimensions>(source, dimension)
    return {
      ...source,
  ...
  public insertSthInCursor (
    ...
    ? target.reduce((iteratee, t) => {
      iteratee = iteratee.concat(t.name === cursor.name ? [t, widgetDimension] : t)
      return iteratee
    }, [])
  ...
  public insertWithSthRowsOrCols (
    ...
    dimension: WidgetDimensions,
    widgetDimension: RequireAtLeastOne<IWidgetDimension, keyof IWidgetDimension>,
    cursor?: Pick<IWidgetDimension, 'name'>): WidgetAbstract {
    const target = this.getTargetPropsByProperty<WidgetAbstract, WidgetDimensions>(source, dimension)
  ...
  public initWidgetNativeFilters () {
    const filters = this.getTargetPropsByProperty<WidgetAbstract, 'filters'>(this.widgetProps, 'filters')
