...
import { displayDefaultState } from 'app/containers/Display/reducer'
import {
  mockGraphLayerId,
  mockSlideId,
  mockEditing,
...
describe('displaySelector', () => {
  ...
  describe('makeSelectLayersBySlide', () => {
    ...
    test('check memoization', () => {
      selectLayersBySlide(state, mockSlideId)
      expect(selectLayersBySlide.recomputations()).toBe(1)
      selectLayersBySlide(state, mockSlideId)
      expect(selectLayersBySlide.recomputations()).toBe(1)
      const newState = {
        ...state,
      ...
      selectLayersBySlide(newState, mockSlideId)
      expect(selectLayersBySlide.recomputations()).toBe(2)
    })
  })
  ...
  describe('makeSelectLayerIdsBySlide', () => {
    ...
    test('check memoization', () => {
      selectLayerIdsBySlide(state, mockSlideId)
      expect(selectLayerIdsBySlide.recomputations()).toBe(1)
      selectLayerIdsBySlide(state, mockSlideId)
      expect(selectLayerIdsBySlide.recomputations()).toBe(1)
      const newState = {
        ...state,
        display: {
          slideLayers: {
            [mockSlideId]: mockSlideSingleGraphLayerFormed
          }
        }
      }
      selectLayerIdsBySlide(newState, mockSlideId)
      expect(selectLayerIdsBySlide.recomputations()).toBe(2)
    })
  })
