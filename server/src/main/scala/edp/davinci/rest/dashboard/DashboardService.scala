...
trait DashboardService {
  ...
  def getDashBoard(dashboardId: Long): Future[Option[PutDashboardInfo]] = {
    val query = modules.dashboardQuery.filter(_.id === dashboardId).map(d => (d.id, d.name, d.pic, d.desc, d.linkage_detail, d.publish, d.active, d.create_by) <> (PutDashboardInfo.tupled, PutDashboardInfo.unapply)).result
    db.run(query.headOption).mapTo[Option[PutDashboardInfo]]
  }


  def update(session: SessionClass, dashboardSeq: Seq[PutDashboardInfo]): Future[Unit] = {
    val query = DBIO.seq(dashboardSeq.map(r => {
      modules.dashboardQuery.filter(obj => obj.id === r.id && obj.create_by === session.userId).map(dashboard => (dashboard.name, dashboard.desc, dashboard.linkage_detail, dashboard.publish, dashboard.update_by, dashboard.update_time))
        .update(r.name, r.desc, r.linkage_detail, r.publish, session.userId, ResponseUtils.currentTime)
    }): _*)
    db.run(query)
  }
  ...
  def getAll(session: SessionClass): Future[Seq[PutDashboardInfo]] = {
    ...
    val query = if (session.admin) modules.dashboardQuery.filter(_.id in dashboardIds).map(d => (d.id, d.name, d.pic, d.desc, d.linkage_detail, d.publish, d.active, d.create_by) <> (PutDashboardInfo.tupled, PutDashboardInfo.unapply)).result
    else modules.dashboardQuery.filter(d => d.publish && (d.id in dashboardIds)).map(d => (d.id, d.name, d.pic, d.desc, d.linkage_detail, d.publish, d.active, d.create_by) <> (PutDashboardInfo.tupled, PutDashboardInfo.unapply)).result
