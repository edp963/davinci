    ...
    protected void tell(MsgWrapper wrapper) {
        ...
        switch (wrapper.getAction()) {
            case DOWNLOAD:
                ...
                if (record == null) {
                    log.error("Download record is null, nothing to do");
                    break;
                }
                if (StringUtils.isNotEmpty(wrapper.getRst())) {
                    record.setStatus(DownloadRecordStatusEnum.SUCCESS.getStatus());
                    record.setPath(wrapper.getRst());
                } else {
                    record.setStatus(DownloadRecordStatusEnum.FAILED.getStatus());
                }
                ((DownloadRecordExtendMapper) SpringContextHolder.getBean(DownloadRecordExtendMapper.class)).updateById(record);
                log.info("Download record is updated status:" + record.getStatus());
                break;
            case MAIL:
            ...
            case SHAREDOWNLOAD:
                ...
                if (shareDownloadRecord == null) {
                    log.error("ShareDownload record is null, nothing to do");
                    break;
                }

                if (StringUtils.isNotEmpty(wrapper.getRst())) {
                    shareDownloadRecord.setStatus(DownloadRecordStatusEnum.SUCCESS.getStatus());
